				...
				content: (
					<div className={"w-full min-w-0 h-full flex"}>
						<ul role="list" className="divide-y divide-gray-100">
							...
							{tests.map((release) => {
								return (
									<div className="flex flex-col" key={release}>
										<li className="flex justify-between gap-x-6 py-5" key={release} onClick={()=>setSelected({
											name: `v0.0.${release}`,
											created_at: new Date(),
											body: `v0.0.${release} test`,
										})}>
											<div className="min-w-0 flex-auto">
												<p className="text-sm font-semibold leading-6 text-gray-900">V0.0.{release}</p>
											</div>
											<div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
												<p className="mt-1 text-xs leading-5 text-gray-500"><time dateTime={releases[0].created_at.toString()}>{releases[0].created_at.toString()}</time></p>
											</div>
											
										</li>
										<Markdown>{releases[0].body}</Markdown>
									</div>
								)
							})}
						</ul>
